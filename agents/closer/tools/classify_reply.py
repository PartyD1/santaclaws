...
def _prompt(inbound: dict[str, Any], lead: dict[str, Any] | None, outreach: dict[str, Any] | None) -> str:
    ...
    return PROMPT_PATH.read_text(encoding="utf-8").format(
        reply_text=inbound.get("raw_content") or inbound.get("transcript") or "",
...
def run(inbound_id: UUID | str) -> dict[str, Any]:
    ...
    try:
        ...
        if inbound.get("channel") != "email":
            result = {"error": "only email inbound rows are supported for Task 29.", "_status": "skipped"}
            _safe_log("classify_reply", "skipped", "skipped non-email inbound reply.", None, result)
            return result
        ...
        _safe_log(
            "classify_reply",
            "succeeded",
            f"classified inbound reply as {result['classification']} ({result['confidence']}%).",
            lead_id,
            result,
        )
        return result
