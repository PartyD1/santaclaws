...
export function MockupPreview({ site }: MockupPreviewProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Score {site.self_critique_score ?? "-"}/10 / {site.self_critique_iterations ?? 1} iteration(s)
          </p>
        </div>
      ...
      <div className="aspect-video overflow-hidden border-y border-slate-100 bg-slate-100">
        {url ? (
          <iframe className="h-full w-full bg-white" src={url} title={`${site.variant} mockup`} />
        ) : site.html_content ? (
          <iframe className="h-full w-full bg-white" srcDoc={site.html_content} title={`${site.variant} mockup`} />
        ) : (
          <div className="flex h-full items-center justify-center text-sm text-slate-500">No preview available.</div>
        )}
      </div>
      <div className="space-y-2 px-4 py-3 text-xs leading-5 text-slate-600">
        {url && (
